...
extension Color {
    ...
    static func interpolate(from start: Color, to end: Color, value: Double) -> Color {
        ...
        let start = ColorComponents(start)
        let end = ColorComponents(end)

        let red = start.red + (end.red - start.red) * frac
        let green = start.green + (end.green - start.green) * frac
        let blue = start.blue + (end.blue - start.blue) * frac
        let alpha = start.opacity + (end.opacity - start.opacity) * frac
